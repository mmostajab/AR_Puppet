...
int subpixSampleSafe ( const cv::Mat &pSrc, cv::Point2f p )
{
...
///						stripePtr += 2*iplStripe->widthStep;
					...
					if (isFirstStripe)
					{
						...
						cv::imshow ( "Exercise 5 - Stripe", iplTmp );//iplStripe );
						isFirstStripe = false;
					}
...
// Added in Exercise 9 - End *****************************************************************
		...
		cv::imshow("Exercise 5 - Original Image", img_bgr);
		cv::imshow("Exercise 5 - Converted Image", img_mono);
